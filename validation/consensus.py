...
from stride.analyzer import GENERIC_COUNTERMEASURES
...
def _description_parts(analyses: list, sources: list) -> list:
    """
    Mesma lógica de _merge_descriptions, mas devolvendo uma lista estruturada
    [{"source": src, "text": desc}, ...] em vez de uma string única concatenada
    com " | ". Usado pelo relatório em PDF para renderizar cada fonte em uma
    linha própria (ver report/templates/stride_report.html) — evita que
    "[Claude] ..." apareça no meio de uma linha de texto corrido, dificultando
    a leitura.
    """
    parts = []
    seen_texts = set()
    for src, a in zip(sources, analyses):
        if a is None or not a.get("applicable"):
            continue
        desc = (a.get("description") or "").strip()
        if desc and desc not in seen_texts:
            seen_texts.add(desc)
            parts.append({"source": src, "text": desc})
    return parts or [{"source": None, "text": "Sem detalhamento disponível."}]
 
 
def _merge_countermeasures(analyses: list, sources: list, category: str, is_applicable: bool) -> list:
    """
    ...
    contramedidas por classe+categoria) preenche esse campo de verdade;
    Claude/OpenAI ficam prontos para contribuir também caso os validators
    passem a retorná-lo no futuro.
 
    Fallback: se a ameaça é aplicável (ex. Claude e/ou OpenAI detectaram algo
    que a heurística do SLM não cobria para aquela classe de componente — o
    próprio motivo de ter validação multi-LLM) mas nenhuma fonte trouxe
    contramedidas específicas, usa a base GENÉRICA por categoria STRIDE em vez
    de deixar a célula vazia — melhor uma recomendação genérica do que nenhuma.
    """
    merged = []
    for _src, a in zip(sources, analyses):
        ...
        for c in a.get("countermeasures") or []:
            if c not in merged:
                merged.append(c)
    if not merged and is_applicable:
        merged = list(GENERIC_COUNTERMEASURES.get(category, []))
    return merged
    ...
    for component_id in component_ids:
        ...
        for category in STRIDE_CATEGORIES:
            ...
            threats[category] = {
                ...
                "description_parts": _description_parts(analyses, SOURCES),
                "severity": _pick_severity(analyses, SOURCES),
                "confidence": confidence,
                "detected_by": detected_by,
                "countermeasures": _merge_countermeasures(analyses, SOURCES, category, is_applicable),
            }
            if is_applicable:
